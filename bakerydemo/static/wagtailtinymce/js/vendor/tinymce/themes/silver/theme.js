...
(function () {
    ...
    const patchSpecChildren = (parent, specs, build) => patchChildrenWith(parent, specs, (spec, index) => {
    ...
    const manager = () => {
      const ariaId = generate$6('aria-owns');
      const link = elem => {
        set$9(elem, 'aria-owns', ariaId);
      };
      const unlink = elem => {
    ...
    const withReuse = (parent, data) => {
      preserve$1(() => {
        virtualReplaceChildren(parent, data, () => {
          return patchSpecChildren(parent.element, data, (d, i) => {
            const optObsoleted = child$2(parent.element, i);
    ...
    const virtualReplace = (component, replacee, _replaceeIndex, childSpec) => {
    ...
    var NotificationManagerImpl = (editor, extras, uiMothership) => {
      ...
      const reposition = notifications => {
        if (notifications.length > 0) {
          each$1(notifications, (notification, index) => {
            if (index === 0) {
    ...
    const register$e = editor => {
      ...
      registerOption('min_height', { processor: 'number' });
      ...
      registerOption('resize', {
        ...
        default: !global$5.deviceType.isTouch() && !editor.hasPlugin('autoresize')
      });
    };
    ...
    const disabled = defaultedBoolean('disabled', false);
    ...
    const renderChoiceItem = (spec, useText, presets, onItemValueHandler, isSelected, itemResponse, providersBackstage, renderIcons = true) => {
      const getApi = component => ({
        ...
        isDisabled: () => Disabling.isDisabled(component),
        setDisabled: state => Disabling.set(component, state)
      });
      const structure = renderItemStructure({
        presets,
    ...
    const fireSkinLoaded$1 = editor => editor.fire('SkinLoaded');
    const fireSkinLoadError$1 = (editor, error) => editor.fire('SkinLoadError', error);
    const fireResizeEditor = editor => editor.fire('ResizeEditor');
    const fireResizeContent = (editor, e) => editor.fire('ResizeContent', e);
    const fireScrollContent = (editor, e) => editor.fire('ScrollContent', e);
    const fireTextColorChange = (editor, data) => editor.fire('TextColorChange', data);
    ...
    const renderUrlInput = (spec, backstage, urlBackstage, initialData) => {
      ...
      const memUrlBox = record({
        ...
        behaviours: derive$1([Disabling.config({ disabled: () => spec.disabled || providersBackstage.isDisabled() })])
      });
    ...
    const renderCheckbox = (spec, providerBackstage, initialData) => {
      ...
      const pField = FormField.parts.field({
        ...
        behaviours: derive$1([
          ComposingConfigs.self(),
          Disabling.config({ disabled: () => spec.disabled || providerBackstage.isDisabled() }),
          Tabstopping.config({}),
          Focusing.config({}),
    ...
    const init$7 = (sink, editor, lazyAnchorbar) => {
    ...
    const generateSelectItems = (_editor, backstage, spec) => {
      const generateItem = (rawItem, response, disabled, value) => {
    ...
    const renderSplitButton = (spec, sharedBackstage) => {
      ...
      const getApi = comp => ({
        ...
        setDisabled: state => Disabling.set(comp, state),
    ...
    const renderElementPath = (editor, settings, providersBackstage) => {
      ...
      const getDataPath = data => {
        const parts = data || [];
        const newPathElements = map$2(parts, (part, index) => Button.sketch({
          dom: {
            tag: 'div',
        ...
        const divider = {
          ...
          components: [text$1(` ${ settings.delimiter } `)]
        };
    ...
    const renderStatusbar = (editor, providersBackstage) => {
      const renderBranding = () => {
        const label = global$8.translate([
          'Powered by {0}',
          'Tiny'
        ]);
    ...
    const setup$3 = editor => {
      ...
      let lazyOuterContainer = Optional.none();
      ...
      const uiContainer = getUiContainer(editor);
      const dirAttributes = global$8.isRtl() ? { attributes: { dir: 'rtl' } } : {};
      ...
      const resizeUiMothership = () => {
        set$8(uiMothership.element, 'width', document.body.clientWidth + 'px');
      };
      const makeSinkDefinition = () => {
        ...
        const sinkSpec = {
          ...
          behaviours: derive$1([Positioning.config({ useFixed: () => isHeaderDocked() })])
        };
      ...
      const sink = build$1(makeSinkDefinition());
      ...
      const backstage = init$7(sink, editor, lazyAnchorBar);
      ...
      const setEditorSize = () => {
        ...
        if (!editor.inline) {
          ...
          if (isValidValue('div', 'height', parsedHeight)) {
            ...
            set$8(outerContainer.element, 'height', '200px');
          }
        }
        return parsedHeight;
      };
    ...
    var Theme = () => {
      global$a.add('silver', editor => {
        ...
        const {uiMothership, backstage, renderUI, getUi} = setup$3(editor);
