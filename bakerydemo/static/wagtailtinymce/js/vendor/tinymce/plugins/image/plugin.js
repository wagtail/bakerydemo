...
(function () {
    ...
    const calcBorderWidth = css => css['border-width'] ? removePixelSuffix(String(css['border-width'])) : '';
    const calcBorderStyle = css => css['border-style'] ? String(css['border-style']) : '';
    const calcStyle = (parseStyle, serializeStyle, css) => serializeStyle(parseStyle(serializeStyle(css)));
    const changeStyle2 = (parseStyle, serializeStyle, data) => {
      const css = mergeMargins(parseStyle(data.style));
    ...
    const submitHandler = editor => info => api => {
      const data = deepMerge(fromImageData(info.image), api.getData());
