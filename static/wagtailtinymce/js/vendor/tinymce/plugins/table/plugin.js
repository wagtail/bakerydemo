...
(function () {
    ...
    const bind$2 = (xs, f) => flatten$1(map$1(xs, f));
    ...
    const isComment = element => type$1(element) === COMMENT || name(element) === '#comment';
    ...
    const children$3 = element => map$1(element.dom.childNodes, SugarElement.fromDom);
    ...
    const children$2 = (scope, predicate) => filter$2(children$3(scope), predicate);
    const descendants$1 = (scope, predicate) => {
      let result = [];
      each$2(children$3(scope), x => {
        if (predicate(x)) {
    ...
    const filterFirstLayer = (scope, selector, predicate) => {
      return bind$2(children$3(scope), x => {
        if (is$2(x, selector)) {
    ...
    const columns$1 = ancestor => {
      if (is$2(ancestor, 'colgroup')) {
        ...
        return bind$2(columnGroups(ancestor), columnGroup => children$1(columnGroup, 'col'));
      }
    };
    ...
    const clone$1 = (gridRow, cloneRow, cloneCell) => {
    ...
    const empty = element => {
      ...
      each$2(children$3(element), rogue => {
        remove$5(rogue);
      });
    };
    ...
    const getOption = element => api$2.getOption(element);
    const set = (element, value) => api$2.set(element, value);
    ...
    const cata$2 = (subject, onNone, onMultiple, onSingle) => subject.fold(onNone, onMultiple, onSingle);
    const none$1 = type.none;
    const multiple = type.multiple;
    const single = type.single;

    const Selections = (lazyRoot, getStart, selectedSelector) => {
      const get = () => retrieve(lazyRoot(), selectedSelector).fold(() => getStart().fold(none$1, single), cells => multiple(cells));
      return { get };
    };
    ...
    const copy$1 = (original, tag) => {
      const nu = shallowAs(original, tag);
      const cloneChildren = children$3(deep(original));
    ...
    const mutate$1 = (original, tag) => {
      const nu = shallowAs(original, tag);
      after$5(original, nu);
      const children = children$3(original);
      append(nu, children);
      remove$5(original);
      return nu;
    };
    ...
    const detect$6 = (versionRegexes, agent) => {
    ...
    const detect$5 = (candidates, userAgent) => {
    ...
    const detectBrowser = (browsers, userAgent) => {
      return detect$5(browsers, userAgent).map(browser => {
    ...
    const detectOs = (oses, userAgent) => {
      return detect$5(oses, userAgent).map(os => {
    ...
    const detect$4 = (userAgent, userAgentDataOpt, mediaMatch) => {
    ...
    const PlatformDetection = { detect: detect$4 };

    const mediaMatch = query => window.matchMedia(query).matches;
    let platform = cached(() => PlatformDetection.detect(navigator.userAgent, Optional.from(navigator.userAgentData), mediaMatch));
    const detect$3 = () => platform();
    ...
    const getOuter$2 = element => api$1.getOuter(element);
    ...
    const get$7 = element => api.get(element);
    const getOuter$1 = element => api.getOuter(element);
    ...
    const detect$2 = onDirection(ltr$1, rtl$1);
    const width = {
      delta: (amount, table) => detect$2(table).delta(amount, table),
      positions: (cols, table) => detect$2(table).positions(cols, table),
      edge: cell => detect$2(cell).edge(cell)
    };
    ...
    const getPercentageWidth = cell => getPercentSize(cell, get$8, getInner);
    const getPixelWidth$1 = cell => isCol$2(cell) ? get$8(cell) : getRuntime$1(cell);
    ...
    const getPixelTableWidth = table => get$8(table) + 'px';
    const getPercentTableWidth = table => getPercentSize(table, get$8, getInner) + '%';
      ...
      return map$1(columns$1, (cellOption, c) => {
        return getDimension(cellOption, c, backups, colFilter, column => {
          if (isValidColumn(column)) {
            ...
            return getDimension(cell, c, backups, colFilter, cell => fallback(Optional.some(get$8(cell))), fallback);
          }
        }, fallback);
      });
    };
    ...
    const noneSize = table => {
      const getWidth = widthLookup(table, get$8);
    ...
    const getEnd = element => name(element) === 'img' ? 1 : getOption(element).fold(() => children$3(element).length, v => v.length);
    ...
    const descendantRtl = (scope, predicate) => {
      const descend = element => {
        const children = children$3(element);
    ...
    const selection = selections => cata$2(selections.get(), constant([]), identity, pure);
    const unmergable = selections => {
    ...
    const mergable = (table, selections, ephemera) => cata$2(selections.get(), Optional.none, cells => {
      if (cells.length <= 1) {
        return Optional.none();
      } else {
    ...
    const forMenu = (selections, table, cell) => ({
      element: cell,
    ...
    const pasteRows = (selections, cell, clipboard, generators) => ({
      selection: selection(selections),
      clipboard,
      generators
    });
    ...
    const getCellsFromSelection = selections => selection(selections);
    const getRowsFromSelection = (selected, selector) => {
      const cellOpt = getSelectionCell(selected);
      const rowsOpt = cellOpt.bind(cell => table(cell)).map(table => rows$1(table));
      return lift2(cellOpt, rowsOpt, (cell, rows) => filter$2(rows, row => exists(fromDom(row.dom.cells), rowCell => get$b(rowCell, selector) === '1' || eq$1(rowCell, cell)))).getOr([]);
    };
    ...
    const registerEvents = (editor, selections, actions) => {
      ...
      editor.on('BeforeSetContent', e => {
        if (e.selection === true && e.paste === true) {
          const selectedCells = getCellsFromSelection(selections);
    ...
    const InDrag = () => {
      ...
      const events = create$4({ move: Event(['info']) });
      return {
        onEvent,
        reset,
    ...
    const handle$2 = (filter, handler) => rawEvent => {
      if (filter(rawEvent)) {
        handler(fromRawEvent$1(rawEvent));
      }
    };
    const binder = (element, event, filter, handler, useCapture) => {
      const wrapped = handle$2(filter, handler);
    ...
    const get$5 = element => read(element, 'class');
    ...
    const cleanClass = element => {
      const classList = supports(element) ? element.dom.classList : get$5(element);
    ...
    const transform$1 = (mutation, settings = {}) => {
      var _a;
    ...
    const BarManager = wire => {
      ...
      const resizing = transform$1(mutation, {});
    ...
    const create$3 = (wire, resizing, lazySizing) => {
    ...
    const TableResize = { create: create$3 };

    const fireNewRow = (editor, row) => editor.fire('newrow', { node: row });
    const fireNewCell = (editor, cell) => editor.fire('newcell', { node: cell });
    ...
    const defaultCellBorderWidths = range$1(5, i => {
      const size = `${ i + 1 }px`;
      return {
        title: size,
        value: size
    ...
    const defaultCellBorderStyles = map$1([
      'Solid',
      'Dotted',
      'Dashed',
      'Double',
    ...
    const determineDefaultStyles = editor => {
      var _a;
    ...
    const register = editor => {
      ...
      registerOption('table_grid', {
        processor: 'boolean',
        default: !global$2.deviceType.isTouch()
      });
    ...
    const hasTabNavigation = option('table_tab_navigation');
    ...
    const useColumnGroup = option('table_use_colgroups');
    ...
    const isPercentagesForced = editor => getTableSizingMode(editor) === 'relative';
    ...
    const isPreserveTableColumnResizing = editor => getColumnResizingBehaviour(editor) === 'preservetable';
    const isResizeTableColumnResizing = editor => getColumnResizingBehaviour(editor) === 'resizetable';
    const getCloneElements = editor => Optional.from(editor.options.get('table_clone_elements'));
    const hasObjectResizing = editor => {
    ...
    const enforcePercentage = convertToPercentSize;
    const enforcePixels = convertToPixelSize;
    const enforceNone = convertToNoneSize;
    ...
    const get$3 = (editor, isResizable) => {
    ...
    const getResizeHandler = editor => {
      let selectionRng = Optional.none();
      let resize = Optional.none();
      ...
      const lazyWire = () => wire.getOr(ResizeWire.only(SugarElement.fromDom(editor.getBody()), isResizable));
      const lazySizing = table => get$4(editor, table);
      ...
      editor.on('init', () => {
        const rawWire = get$3(editor, isResizable);
    ...
    const findCommonRowType = rows => {
      const rowTypes = map$1(rows, row => getRowType$1(row).type);
    ...
    const findTableRowHeaderType = warehouse => findMap(warehouse.all, row => {
      const rowType = getRowType$1(row);
    ...
    const insert$1 = (table, selector, element) => {
      last$2(children$1(table, selector)).fold(() => prepend(table, element), child => after$5(child, element));
    };
    const generateSection = (table, sectionName) => {
      const section = child$1(table, sectionName).getOrThunk(() => {
    ...
    const run = (operation, extract, adjustment, postAction, genWrappers) => (wire, table, target, generators, behaviours) => {
      const warehouse = Warehouse.fromTable(table);
    ...
    const rowFill = (grid, amount, generators, lockedColumns) => {
      ...
      return grid.concat(range$1(amount, () => {
        ...
        const row = clone$1(exampleRow, generator, identity);
    ...
    const cloneRow = (row, cloneCell, comparator, substitution) => clone$1(row, elem => substitution(elem, comparator), cloneCell);
    ...
    const merge = cells => {
      ...
      const markContent = () => {
        const content = bind$2(cells, cell => {
          const children = children$3(cell);
    ...
    const opGetColumnsType = (table, target) => {
      ...
      return details.bind(selectedCells => {
        ...
        const selectedColumnCells = flatten$1(map$1(house.all, row => filter$2(row.cells, cell => cell.column >= minColRange && cell.column < maxColRange)));
    ...
    const TableActions = (editor, cellSelection, lazyWire) => {
      ...
      const execute = (operation, guard, mutate, lazyWire, effect) => (table, target, noEvents = false) => {
        removeDataStyle(table);
        const wire = lazyWire();
        const doc = SugarElement.fromDom(editor.getDoc());
      ...
      const deleteRow = execute(eraseRows, lastRowGuard, noop, lazyWire, structureModified);
      const deleteColumn = execute(eraseColumns, lastColumnGuard, noop, lazyWire, structureModified);
      const insertRowsBefore$1 = execute(insertRowsBefore, always, noop, lazyWire, structureModified);
      const insertRowsAfter$1 = execute(insertRowsAfter, always, noop, lazyWire, structureModified);
      const insertColumnsBefore$1 = execute(insertColumnsBefore, always, colMutationOp, lazyWire, structureModified);
      const insertColumnsAfter$1 = execute(insertColumnsAfter, always, colMutationOp, lazyWire, structureModified);
      const mergeCells$1 = execute(mergeCells, always, noop, lazyWire, structureModified);
      const unmergeCells$1 = execute(unmergeCells, always, noop, lazyWire, structureModified);
      const pasteColsBefore$1 = execute(pasteColsBefore, always, noop, lazyWire, structureModified);
      const pasteColsAfter$1 = execute(pasteColsAfter, always, noop, lazyWire, structureModified);
      const pasteRowsBefore$1 = execute(pasteRowsBefore, always, noop, lazyWire, structureModified);
      const pasteRowsAfter$1 = execute(pasteRowsAfter, always, noop, lazyWire, structureModified);
      const pasteCells$1 = execute(pasteCells, always, noop, lazyWire, styleAndStructureModified);
      const makeCellsHeader$1 = execute(makeCellsHeader, always, noop, lazyWire, structureModified);
      const unmakeCellsHeader$1 = execute(unmakeCellsHeader, always, noop, lazyWire, structureModified);
      const makeColumnsHeader$1 = execute(makeColumnsHeader, always, noop, lazyWire, structureModified);
      const unmakeColumnsHeader$1 = execute(unmakeColumnsHeader, always, noop, lazyWire, structureModified);
      const makeRowsHeader$1 = execute(makeRowsHeader, always, noop, lazyWire, structureModified);
      const makeRowsBody$1 = execute(makeRowsBody, always, noop, lazyWire, structureModified);
      const makeRowsFooter$1 = execute(makeRowsFooter, always, noop, lazyWire, structureModified);
    ...
    const get$2 = element => element.dom.innerHTML;
    ...
    const insertTableWithDataValidation = (editor, rows, columns, options = {}, errorMsg) => {
    ...
    const getClipboardElements = getClipboard => () => getClipboard().fold(() => [], elems => map$1(elems, e => e.dom));
    const setClipboardElements = setClipboard => elems => {
      const elmsOpt = elems.length > 0 ? Optional.some(fromDom(elems)) : Optional.none();
      setClipboard(elmsOpt);
    };
    const insertTable = editor => (columns, rows, options = {}) => {
    ...
    const getApi = (editor, clipboard, resizeHandler, selectionTargets) => ({
    ...
    const onSetupToggle = (editor, selections, formatName, formatValue) => {
      return api => {
    ...
    const buildListItems = items => map$1(items, item => {
    ...
    const buildMenuItems = (editor, selections, items, format, onAction) => map$1(items, item => {
      const text = item.text || item.title;
      if (isListGroup(item)) {
        return {
          type: 'nestedmenuitem',
    ...
    const filterNoneItem = list => bind$2(list, item => {
      if (isListGroup(item)) {
        return [{
            ...item,
    ...
    const generateMenuItemsCallback = (editor, selections, items, format, onAction) => callback => callback(buildMenuItems(editor, selections, items, format, onAction));
    const buildColorMenu = (editor, colorList, style) => {
      const colorMap = map$1(colorList, entry => ({
        text: entry.title,
    ...
    const extractAdvancedStyles = elm => {
      ...
      return {
        ...
        bordercolor: getRaw$2(element, 'border-color').map(rgbToHex).getOr(''),
        backgroundcolor: getRaw$2(element, 'background-color').map(rgbToHex).getOr('')
      };
    };
    const getSharedValues = data => {
      const baseData = data[0];
      const comparisonData = data.slice(1);
      each$2(comparisonData, items => {
        each$2(keys(baseData), key => {
    ...
    const getAlignment = (formats, formatName, editor, elm) => find$1(formats, name => !isUndefined(editor.formatter.matchNode(elm, formatName + name))).getOr('');
    ...
    const extractDataFromSettings = (editor, hasAdvTableTab) => {
      ...
      const extractAdvancedStyleData = () => ({
        ...
        bordercolor: rgbToHex(get$c(style, 'border-color').getOr('')),
        backgroundcolor: rgbToHex(get$c(style, 'background-color').getOr(''))
      });
      const defaultData = {
        height: '',
        width: '100%',
      ...
      const getBorder = () => {
        ...
        return get$c(attrs, 'border').fold(() => ({}), border => ({ border }));
      };
      const advStyle = hasAdvTableTab ? extractAdvancedStyleData() : {};
      const getCellPaddingCellSpacing = () => {
        const spacing = get$c(style, 'border-spacing').or(get$c(attrs, 'cellspacing')).fold(() => ({}), cellspacing => ({ cellspacing }));
        const padding = get$c(style, 'border-padding').or(get$c(attrs, 'cellpadding')).fold(() => ({}), cellpadding => ({ cellpadding }));
        return {
          ...spacing,
          ...padding
        };
    ...
    const extractDataFromTableElement = (editor, elm, hasAdvTableTab) => {
      const getBorder = (dom, elm) => {
        const optBorderWidth = getRaw$2(SugarElement.fromDom(elm), 'border-width');
    ...
    const getSelectedCells = (table, cells) => {
      ...
      const filtered = filter$2(allCells, cellA => exists(cells, cellB => eq$1(cellA.element, cellB)));
      return map$1(filtered, cell => ({
    ...
    const applyCellData = (editor, cells, oldData, data) => {
      ...
      if (size(modifiedData) > 0 && cells.length >= 1) {
        table(cells[0]).each(table => {
          ...
          const structureModified = has$1(modifiedData, 'celltype');
    ...
    const getData = (editor, cells) => {
      const cellsData = table(cells[0]).map(table => map$1(getSelectedCells(table, cells), item => extractDataFromCellElement(editor, item.element, hasAdvancedCellTab(editor), item.column)));
      return getSharedValues(cellsData.getOrDie());
    };
    const open$2 = (editor, selections) => {
      const cells = getCellsFromSelection(selections);
      if (cells.length === 0) {
        return;
      }
    ...
    const applyRowData = (editor, rows, oldData, data) => {
      ...
      if (size(modifiedData) > 0) {
        const typeModified = has$1(modifiedData, 'type');
    ...
    const open$1 = editor => {
      ...
      const rowsData = map$1(rows, rowElm => extractDataFromRowElement(editor, rowElm.dom, hasAdvancedRowTab(editor)));
      ...
      editor.windowManager.open({
        ...
        initialData: data,
        onSubmit: curry(onSubmitRowForm, editor, map$1(rows, r => r.dom), data)
      });
    };
    ...
    const onSubmitTableForm = (editor, tableElm, oldData, api) => {
      ...
      editor.undoManager.transact(() => {
        ...
        editor.addVisual();
        if (size(modifiedData) > 0) {
          const captionModified = has$1(modifiedData, 'caption');
    ...
    const registerCommands = (editor, actions, cellSelection, selections, clipboard) => {
      ...
      const actOnSelection = (execute, noEvents = false) => performActionOnSelection((table, startCell) => {
        const targets = forMenu(selections, table, startCell);
      ...
      const pasteOnSelection = (execute, getRows) => getRows().each(rows => {
        ...
        performActionOnSelection((table, startCell) => {
          ...
          const targets = pasteRows(selections, startCell, clonedRows, generators);
    ...
    const none = current => adt$5.none(current);
    const CellLocation = {
      ...adt$5,
      none
    };
    ...
    const detect$1 = (current, isRoot) => {
      return table(current, isRoot).bind(table => {
        const all = cells$1(table);
        const index = findIndex(all, x => eq$1(current, x));
    ...
    const next = (current, isEligible, isRoot) => {
      const detection = detect$1(current, isRoot);
    ...
    const exactFromRange = simRange => adt$3.exact(simRange.start, simRange.soffset, simRange.finish, simRange.foffset);
    const getStart = selection => selection.match({
      domRange: rng => SugarElement.fromDom(rng.startContainer),
      relative: (startSitu, _finishSitu) => Situ.getStart(startSitu),
      exact: (start, _soffset, _finish, _foffset) => start
    });
    const domRange = adt$3.domRange;
    const relative = adt$3.relative;
    const exact = adt$3.exact;
    ...
    const doDiagnose = (win, ranges) => {
      ...
      if (rng.collapsed) {
        ...
        return reversed.map(rev => adt$2.rtl(SugarElement.fromDom(rev.endContainer), rev.endOffset, SugarElement.fromDom(rev.startContainer), rev.startOffset)).getOrThunk(() => fromRange(win, adt$2.ltr, rng));
      } else {
    ...
    const get$1 = win => getExact(win).map(range => SimSelection.exact(range.start, range.soffset, range.finish, range.foffset));
    ...
    const forward = (editor, isRoot, cell) => go$1(editor, isRoot, next(cell, isEditable$1));
    const backward = (editor, isRoot, cell) => go$1(editor, isRoot, prev(cell, isEditable$1));
    const getCellFirstCursorPosition = (editor, cell) => {
      const selection = SimSelection.exact(cell, 0, cell, 0);
      return toNative(selection);
    };
    const go$1 = (editor, isRoot, cell) => {
      return cell.fold(Optional.none, Optional.none, (current, next) => {
    ...
    const handle$1 = (event, editor, cellSelection) => {
      if (event.keyCode === global.TAB) {
        const body = getBody(editor);
        const isRoot = element => {
          const name$1 = name(element);
          return eq$1(element, body) || contains$2(rootElements, name$1);
        };
        const rng = editor.selection.getRng();
        const container = SugarElement.fromDom(event.shiftKey ? rng.startContainer : rng.endContainer);
        cell(container, isRoot).each(cell => {
          event.preventDefault();
          table(cell, isRoot).each(cellSelection.clear);
    ...
    const indexInParent = element => parent(element).bind(parent => {
      const children = children$3(parent);
    ...
    const findBr = (element, offset) => {
      return child$3(element, offset).filter(isBr).orThunk(() => {
    ...
    const WindowBridge = win => {
      ...
      const collapseSelection = (toStart = false) => {
        get$1(win).each(sel => sel.fold(rng => rng.collapse(toStart), (startSitu, finishSitu) => {
      ...
      const getScrollY = () => {
        const pos = get(SugarElement.fromDom(win.document));
        return pos.top;
      };
      const scrollBy = (x, y) => {
    ...
    var CellSelection = (editor, lazyResize, selectionTargets) => {
      const onSelection = (cells, start, finish) => {
        selectionTargets.targets().each(targets => {
          ...
          tableOpt.each(table => {
            ...
            const otherCells = getOtherCells(table, targets, generators);
    ...
    const getSelectionTargets = (editor, selections) => {
      ...
      const resetTargets = () => {
        ...
        each$2(changeHandlers.get(), handler => handler());
      };
      const setupHandler = handler => {
        handler();
        changeHandlers.set(changeHandlers.get().concat([handler]));
        return () => {
          changeHandlers.set(filter$2(changeHandlers.get(), h => h !== handler));
        };
      };
    ...
    const addButtons = (editor, selections, selectionTargets, clipboard) => {
      ...
      if (tableClassList.length !== 0) {
        editor.ui.registry.addMenuButton('tableclass', {
          ...
          fetch: generateMenuItemsCallback(editor, selections, tableClassList, 'tableclass', value => editor.execCommand('mceTableToggleClass', false, value)),
          onSetup: selectionTargets.onSetupTable
        });
      }
      ...
      if (tableCellClassList.length !== 0) {
        editor.ui.registry.addMenuButton('tablecellclass', {
          ...
          tooltip: 'Cell styles',
          fetch: generateMenuItemsCallback(editor, selections, tableCellClassList, 'tablecellclass', value => editor.execCommand('mceTableCellToggleClass', false, value)),
          onSetup: selectionTargets.onSetupCellOrRow
        });
      }
    ...
    const addMenuItems = (editor, selections, selectionTargets, clipboard) => {
      const cmd = command => () => editor.execCommand(command);
      ...
      if (tableClassList.length !== 0) {
        editor.ui.registry.addNestedMenuItem('tableclass', {
          icon: 'table-classes',
          text: 'Table styles',
          getSubmenuItems: () => buildMenuItems(editor, selections, tableClassList, 'tableclass', value => editor.execCommand('mceTableToggleClass', false, value)),
          onSetup: selectionTargets.onSetupTable
        });
      }
      const tableCellClassList = filterNoneItem(getCellClassList(editor));
      if (tableCellClassList.length !== 0) {
        editor.ui.registry.addNestedMenuItem('tablecellclass', {
          icon: 'table-cell-classes',
          text: 'Cell styles',
          getSubmenuItems: () => buildMenuItems(editor, selections, tableCellClassList, 'tablecellclass', value => editor.execCommand('mceTableCellToggleClass', false, value)),
          onSetup: selectionTargets.onSetupCellOrRow
        });
      }
      editor.ui.registry.addNestedMenuItem('tablecellvalign', {
        icon: 'vertical-align',
        text: 'Vertical align',
        getSubmenuItems: () => buildMenuItems(editor, selections, verticalAlignValues, 'tablecellverticalalign', applyTableCellStyle(editor, 'vertical-align')),
        onSetup: selectionTargets.onSetupCellOrRow
      });
      editor.ui.registry.addNestedMenuItem('tablecellborderwidth', {
        icon: 'border-width',
        text: 'Border width',
        getSubmenuItems: () => buildMenuItems(editor, selections, getTableBorderWidths(editor), 'tablecellborderwidth', applyTableCellStyle(editor, 'border-width')),
        onSetup: selectionTargets.onSetupCellOrRow
      });
      editor.ui.registry.addNestedMenuItem('tablecellborderstyle', {
        icon: 'border-style',
        text: 'Border style',
        getSubmenuItems: () => buildMenuItems(editor, selections, getTableBorderStyles(editor), 'tablecellborderstyle', applyTableCellStyle(editor, 'border-style')),
        onSetup: selectionTargets.onSetupCellOrRow
      });
    ...
    const Plugin = editor => {
      register(editor);
      const selections = Selections(() => getBody(editor), () => getSelectionCell(getSelectionStart(editor), getIsRoot(editor)), ephemera.selectedSelector);
